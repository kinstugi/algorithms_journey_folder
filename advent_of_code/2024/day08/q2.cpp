...
void processPoints(pair<int, int> p1, pair<int, int> p2, int R, int C, set<pair<int, int>> &seen, vector<string> &grid){
    ...
    for(const auto &item: table){
        ...
        for (int i = 0; i < sz; i++){
            for (int k = i+1; k < sz; k++){
                processPoints(item.second[i], item.second[k], R, C, seen, grid);
            }
        }
    }
